...
export default function HackerWallpaper({ theme, onClick }: WallpaperProps) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Faint background hex texture (right of icon strip)
      ctx.save();
      ctx.beginPath();
      ctx.rect(90, 0, W - 90, H);
      ctx.clip();
      ...
      const iconStrip = 90;    // width reserved for the icon sidebar
      const cw  = [0.33, 0.33, 0.34];  // column widths as fraction of usable W
      const rh  = [0.34, 0.33, 0.33];  // row heights as fraction of H
      const UW  = W - iconStrip;       // usable width (right of icon strip)
      ...
      // Gutter centre lines (for PCB traces) — skip the icon strip area
